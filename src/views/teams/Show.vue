            ...
            <div class="-ml-4 -mt-2 flex items-center justify-between flex-wrap sm:flex-no-wrap">
              ...
              <div class="ml-4 mt-2 flex-shrink-0">
                <span class="inline-flex rounded-md shadow-sm">
                  <button @click="showCreatePlayer" type="button" class="relative inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-700 active:bg-indigo-700">
                    Add player
                  </button>
                </span>
              </div>
...
export default defineComponent({
  setup() {
    const team = ref(store.state.teams[0]);
    ...
    const showEditPlayer = (id: number): void => {
      router.push({ path: `/teams/${team.value.id}/players/${id}/edit` });
    };
